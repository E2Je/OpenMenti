...
/**
 * Presenter-side renderer. `captureId` is set on the visual container so the PDF
 * engine can snapshot exactly the chart/cloud area at high resolution.
 */
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-10 px-12 py-10 text-center">
      ...
      {slide.type === "word_cloud" && (
        <div
          id="om-capture"
          className="flex min-h-[40vh] w-full max-w-5xl items-center justify-center rounded-card bg-surface p-8"
        >
...
function TextView({ content }: { content: TextContent }) {
  // Minimal, safe markdown: split into lines, bold **..**, bullet lines.
...
function inlineBold(s: string): string {
  // Escape HTML, then apply **bold**. Prevents XSS from free text.
  const esc = s
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
